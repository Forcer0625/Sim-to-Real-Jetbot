...
class ReplayBuffer(object):
    ...
    def store_transition(self, state, action, reward, next_state, terminal, truncate):
...
class N_Steps_ReplayBuffer(object):
    def __init__(self, args):
        ...
        self.buffer = {'state': np.zeros((self.buffer_capacity, *args.state_dim), dtype=np.uint8),
                       'action': np.zeros((self.buffer_capacity, 1)),
                       'reward': np.zeros(self.buffer_capacity),
                       'next_state': np.zeros((self.buffer_capacity, *args.state_dim), dtype=np.uint8),
                       'terminal': np.zeros(self.buffer_capacity),
                       }

    def store_transition(self, state, action, reward, next_state, terminal, truncate):
        transition = (state, action, reward, next_state, terminal, truncate)
    ...
    def sample(self, total_steps):
        ...
        for key in self.buffer.keys():  # numpy->tensor
            if key == 'action':
                batch[key] = torch.tensor(self.buffer[key][index], dtype=torch.long, device=self.device)
            else:
                batch[key] = torch.tensor(self.buffer[key][index], dtype=torch.float32, device=self.device)
...
class Prioritized_ReplayBuffer(object):
    ...
    def sample(self, total_steps):
        ...
        return batch, batch_index, IS_weight.to(self.device)
...
class N_Steps_Prioritized_ReplayBuffer(object):
    ...
    def get_n_steps_transition(self):
        ...
        for i in reversed(range(self.n_steps)):  # 逆序计算n_steps_reward
            r, s_, ter, d = self.n_steps_deque[i][2:]
            n_steps_reward = r + self.gamma * (1 - d) * n_steps_reward
            if d:  # 如果truncate=True，说明一个回合结束，保存deque中当前这个transition的s'和terminal作为这个n_steps_transition的next_state和terminal
                next_state, terminal = s_, ter
